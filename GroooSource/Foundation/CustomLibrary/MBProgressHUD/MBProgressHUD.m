...
- (void)layoutSubviews {
	...
	CGFloat maxWidth = bounds.size.width - 4 * xMargin;
	...
	CGFloat remainingHeight = bounds.size.height - totalSize.height - kPadding - 4 * yMargin;
    ...
    totalSize.width += 2 * (self.mode == MBProgressHUDModeIndeterminate ? MIN(xMargin, yMargin) : xMargin);
	totalSize.height += 2 * (self.mode == MBProgressHUDModeIndeterminate ? MIN(xMargin, yMargin) : yMargin);
	
	// Position elements
	CGFloat yPos = round(((bounds.size.height - totalSize.height) / 2)) + yMargin + yOffset;
  ...
  /*          修改结束         */
	...
	if (square) {
		...
		if (max <= bounds.size.width - 2 * xMargin) {
			totalSize.width = max;
		}
		if (max <= bounds.size.height - 2 * yMargin) {
